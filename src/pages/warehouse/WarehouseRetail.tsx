...
import { Search, Filter, Download, Eye, ShoppingCart, Clock, CheckCircle, XCircle, Package, Truck, TrendingUp, User, Mail, Phone, ShoppingBag, Receipt } from 'lucide-react';
...
const statusConfig: Record<string, { color: string; icon: React.ElementType }> = {
  ...
  packed: { color: 'bg-accent text-accent-foreground', icon: Package },
...
export default function WarehouseRetail() {
  const { user } = useAuth();
  ...
  const fetchRetailOrders = async () => {
    try {
      ...
      console.error("Error fetching retail orders:", err);
    }
  };

  const handleCancelOrder = async (orderId: string) => {
    if (!confirm('Are you sure you want to cancel this retail order?')) return;
    try {
      ...
      if (error) throw error;
      toast.success('Retail order cancelled');
      fetchRetailOrders();
    } catch (err: any) {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `retail_orders_${new Date().getTime()}.csv`);
  ...
  const stats = {
    ...
    avgValue: avgOrderSize > 1000 ? `₹${avgOrderSize.toLocaleString('en-IN', {maximumFractionDigits: 0})}` : `₹${avgOrderSize}`,
    deliveredToday: orders.filter(o => o.status === 'delivered').length,
    activeOrders: orders.filter(o => ['pending', 'processing', 'packed', 'shipped'].includes(o.status)).length,
  };
    ...
    <DashboardLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-display font-bold">Retail Orders</h1>
            <p className="text-muted-foreground mt-1">Manage individual consumer orders and deliveries</p>
          </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle>Retail Order Log</CardTitle>
                <CardDescription>Individual consumer orders and fulfillment</CardDescription>
              </div>
                  ...
                  filtered.map((order) => {
                    ...
                    return (
                      <TableRow key={order.id} className="group">
                        <TableCell className="font-mono font-medium">RTO-{order.id.substring(0,6)}</TableCell>
                        <TableCell>
                          <div className="flex flex-col">
                            <span className="font-medium">{order.profiles?.full_name || 'System User'}</span>
        ...
        <DialogContent className="sm:max-w-lg overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription className="font-mono">
              RTO-{selectedOrder?.id.substring(0, 12).toUpperCase()}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-3 p-4 bg-primary/5 rounded-xl border border-primary/10">
                <p className="text-[10px] font-bold text-primary uppercase tracking-widest flex items-center gap-1.5">
                  <Truck className="h-3 w-3" /> Delivery Context
                </p>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-muted-foreground">Channel</span>
                    <Badge variant="outline" className="text-[10px] uppercase font-bold text-primary border-primary/20">Retail</Badge>
                  </div>
                  <div className="flex justify-between items-center">
              ...
              <div className="p-4 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{selectedOrder?.quantity} units individual purchase</p>
                  </div>
